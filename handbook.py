...
handbooks = _config[0]['handbooks']

# Updating product name
for h in handbooks:
  _custom_config = open("./"+h['path']+"/_config.yml", "r", encoding="utf-8")
  custom_config = list(yaml.load_all(_custom_config, Loader=yaml.FullLoader))
  for lines in custom_config:
    lines['title'] = h['product_name']
    lines['URL_STUDENT'] = h['URL_STUDENT']
    lines['URL_GITHUB'] = h['URL_GITHUB']
    lines['URL_LANDING'] = h['URL_LANDING']
    lines['URL_FACEBOOK'] = h['URL_FACEBOOK']
    lines['URL_LINKEDIN'] = h['URL_LINKEDIN']
    lines['URL_INSTAGRAM'] = h['URL_INSTAGRAM']
    lines['URL_GITHUB_MAIN'] = h['URL_GITHUB_MAIN']
    with open("./"+h['path']+"/_config.yml", "w") as f:
      yaml.dump(lines, f)
#
...
if not os.path.isdir("./"+falias):  # not _page folder mkdir
    os.makedirs("./"+falias)
# The walk method will list all the directories and files under the specified path (including subdirectories and files under subdirectories)
allList = os.walk(mainPath)

# List all subdirectories and all files under the subdirectories
for root, dirs, files in allList:
   # List the path currently read
     # print("path:", root)
     # List the folders read under this path (the second level will only be read after the first level is read)
     # print("directory:", dirs)
     # List all files read under this path
     # print("files:", files)
    ...
    for i in files:
        for h in handbooks:
            ...
            move_path = root.replace(
                df_filepath, "./"+h['product_name']+"/_default") if root == df_filepath else root.replace(df_filepath, "./"+h['product_name']+"/_default")
            move_file = os.path.join(
                move_path, i) if move_path != "./"+h['product_name']+"/_default/" else "./"+h['product_name']+"/_default/"+i
...
# Updating scss
for h in handbooks:
  this_file = os.path.join("./config/"+h['product_name']+"/style.scss")
  move_path = os.path.join("./"+h['product_name']+"/assets/css/style.scss")
  move_file = os.path.join("./"+h['product_name']+"/assets/css/style.scss")
  print(this_file+" ===Updating css from config===> "+move_file)
  copyfile(this_file, move_path)

# Updating index.md product name
for h in handbooks:
  # Read in the file
  with open('./_main/index.md', 'r') as file :
    filedata = file.read()
